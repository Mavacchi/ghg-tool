...
|---|---|---|---|
| Gas Naturale (GAS_NAT, Sm³) — combustion | **IN** | All 7 | 6 native + 1 ETL-synthesised zero (VIANO_GARGOLA = 0, user-confirmed 2026-05-13, no gas connection in 2024) / 7 records (VIANO_GARGOLA = 11 Sm³ confirmed as commissioning value, user 2026-05-13) |
...
**Scope 1 raw record count**: 31 native rows in `scope1_combustione.csv` (header excluded) + 1 ETL-synthesised row (VIANO_GARGOLA GAS_NAT 2024 = 0 Sm³, provenance `auto_zero_user_confirmed`) = 32 total in staging. 2024: 15 native + 1 synthesised = 16 site/fuel records. 2025: 16 native records covering all 7 sites for all applicable fuels.

**Notable 2024 data point**: VIANO_GARGOLA gas connection not active in 2024 → ETL synthesises explicit zero row. **Notable 2025 data point**: VIANO_GARGOLA Gas_Naturale 2025 = 11 Sm³ (commissioning value, confirmed real by user 2026-05-13). DQ-CRIT-01/04/05 closed by this resolution. Annotation in dashboard/PDF: "VIANO_GARGOLA — gas commissioned in 2025".
...
|---|---|---|---|---|
| Cat 1 — Purchased goods and services | Argille, Feldspati, Sabbie silicee, Fritte/smalti, Pigmenti, Additivi chimici, Imballaggi cartone, Pallet legno, Film plastico, Reggette PP, Servizi vari (4 lines) | Mass-based + spend-based | 14 records | 14 records |
...
**Total Scope 3 raw record count**: 100 native data rows across 2024 and 2025 in `scope3_categorie.csv` (header excluded). 9 categories present across both years. Note: 10 Cat 3 WTT/T&D rows have empty Fonte_Dato/Qualità_Dato/Stato_Dato in the native CSV — ETL applies defaulting rule per FR-37 (Fonte="Derivato da Scope 1/2", Qualità="D"=Derivato, Stato="Definitivo") at ingestion.
...
|---|---|---|---|---|---|
| FR-01 | Raw data ingestion — Scope 1 | Ingest `scope1_combustione.csv` (semicolon-delimited, UTF-8 BOM). Validate schema using pandera against the full set of columns: `Scope, Anno, Codice_Sito, Categoria_S1, Combustibile, Quantità, Unità, Fonte_Dato, Qualità_Dato, Stato_Dato, Note`. Mandatory: first 7 columns + Fonte_Dato + Qualità_Dato + Stato_Dato (Note optional). Allowed Codice_Sito values, Categoria_S1/Combustibile pairs, non-negative Quantità. ETL auto-inserts an explicit `VIANO_GARGOLA GAS_NAT 2024 = 0 Sm³` row with provenance `auto_zero_user_confirmed` (user 2026-05-13 — gas connection not active in 2024); without this synth, DQ-CRIT-01/04/05 would fire. Reject invalid rows to DLQ with error code. | All 31 native rows loaded + 1 ETL-synthesised zero row = 32 total in staging; idempotent on re-run; no silent data loss. | MUST | data-engineer |
| FR-02 | Raw data ingestion — Scope 2 | Ingest `scope2_elettricita.csv` (semicolon-delimited, UTF-8 BOM). Full schema: `Scope, Anno, Codice_Sito, Voce_S2, Quantità, Unità, Strumento_MB, Fonte_Dato, Qualità_Dato, Stato_Dato, Note`. Mandatory: all except Note. Enforce allowed Voce_S2 ∈ {EE_Acquistata_GO, EE_Acquistata_Grid}. ETL auto-inserts an explicit `EE_Acquistata_Grid = 0 kWh` row for SASSUOLO 2025 (user-confirmed 100% GO contract switch) with provenance `auto_zero_user_confirmed`, so DQ-CRIT-05 temporal-gap does not fire. Annotate VIANO 2025 ~50% kWh reduction as DQ-WARN-01 (now reclassified as **operational annotation**, not anomaly — fermo parziale confirmed by user 2026-05-13). | All 15 native rows loaded + 1 ETL-synthesised SASSUOLO 2025 Grid zero row = 16 total; provenance tag retained; idempotent on re-run. | MUST | data-engineer |
| FR-03 | Raw data ingestion — Scope 3 | Ingest `scope3_categorie.csv` (semicolon-delimited, UTF-8 BOM). Full schema: `Scope, Anno, Categoria_S3, Sottocategoria, Metodo, Combustibile, Quantità, Unità, Fonte_Dato, Qualità_Dato, Stato_Dato, Note`. Mandatory: first 5 + Quantità + Unità + Fonte_Dato + Qualità_Dato + Stato_Dato (Note optional; Combustibile structurally blank for non-fuel rows e.g. electricity Cat 3). Allow zero-quantity rows (Cat 4 rail entries Feldspati_Treno / Sabbie_Treno) as valid explicit disclosures; enforce allowed Categoria_S3 ∈ {1..15}. Apply FR-37 defaulting rule for the 10 Cat 3 WTT/T&D rows with blank metadata. | All 100 native rows loaded; zero-tkm records retained, not filtered; Cat 3 metadata defaulting applied per FR-37; idempotent on re-run. | MUST | data-engineer |
...
| FR-11 | Scope 3 Cat 3 calculation | Calculate WTT and T&D loss emissions for Gas Naturale, Gasolio, Benzina (DEFRA WTT factors) and Electricity (DEFRA/IEA WTT + T&D factors). **Fuel input quantities derived from Σ Scope 1 fuel data per facility/year (NOT from Cat 3 CSV `Quantità` column).** Electricity input = Σ kWh consumed per Scope 2 (LB basis). Cat 3 CSV `Quantità` column retained as informational reconciliation reference; ETL logs delta (CSV − Σ Scope 1) to `dq_findings` as audit-trail evidence even when delta = 0. **Phase 3 verified current data delta = 0 across all fuel/year combinations** (e.g. Σ Scope 1 GAS_NAT 2024 = 33,149,422 Sm³ = Cat 3 CSV value; identical for 2025 and other fuels; the +18% delta cited in v1.1.0 was a Phase 1 requirements-agent arithmetic error, not a real data discrepancy). The Σ Scope 1 rule remains preventative for future data revisions. | WTT factors from DEFRA current year; T&D loss factor from ISPRA/IEA; WTT fuel calculated on Σ Scope 1 fuel quantity per facility/year; WTT Electricity calculated on total kWh consumed (LB basis); source-of-truth reconciliation rule documented in methodology.md; delta logged in `dq_findings` regardless of value. | MUST | data-analyst |
...
| FR-37 | Cat 3 metadata defaulting on ingestion | For Cat 3 WTT and T&D rows in `scope3_categorie.csv` with blank Fonte_Dato/Qualità_Dato/Stato_Dato (10 rows confirmed in Phase 3 DQ scan), ETL applies deterministic defaults at ingestion: `Fonte_Dato = "Derivato da Scope 1/2 per FR-11"`, `Qualità_Dato = "D"` (Derivato), `Stato_Dato = "Definitivo"`. Defaulting is logged in `dq_findings` as INFO-level (not DQ-CRIT) once applied. Native CSV is NOT modified — defaulting is a runtime-only transform. | DQ-CRIT-02 no longer fires for these 10 rows post-defaulting; the `dq_findings` table records 1 INFO entry per defaulted row with `rule_id='FR-37-DEFAULT'`. | MUST | data-engineer |
...
| OI-2 | **CLOSED 2026-05-13** — Phase 3 + user confirmation: VIANO 2025 -50% pattern (GAS_NAT, GASOLIO, EE_GO all ~-50%) is **fermo parziale reale** (real partial production halt), not data incompleteness. VIANO_GARGOLA GAS_NAT 2025 = 11 Sm³ confirmed as commissioning value (gas connected late 2024 / commissioned 2025). DQ-WARN-01/06/07 reclassified as operational annotations; dashboard displays "VIANO reduced operation 2025" banner. | data-quality-agent + user | Closed | No |
...
| AC-16 | VIANO_GARGOLA GAS_NAT 2024 = 0 real (no gas connection); 2025 = 11 Sm³ commissioning real; FR-01 ETL synthesises 2024 zero row. | Confirmed by user 2026-05-13 |
| AC-17 | Cat 3 metadata defaulting rule (FR-37) closes DQ-CRIT-02; native CSV preserved. | Confirmed by user 2026-05-13 |
| AC-18 | VIANO 2025 -50% energy pattern across 3 carriers is fermo parziale reale (OI-2 closed); dashboard annotates "reduced operation". | Confirmed by user 2026-05-13 |
| AC-19 | SASSUOLO BENZINA 2025 = 9,151 L confirmed as real fleet expansion (vs 1,349 L 2024). | Confirmed by user 2026-05-13 |
...
| FR-37 | Cat 3 metadata defaulting | — | Assurance basis — completeness of audit trail metadata | MUST |
