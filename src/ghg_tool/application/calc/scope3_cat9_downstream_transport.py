"""Scope 3 — Cat 9 Downstream Transportation and Distribution (FR-16).

Distance-based × DEFRA freight by mode (HGV / sea).  Load factor 27 t
recorded in disclosure_notes (per FR-16 acceptance).
"""
...
_LOAD_FACTOR_NOTE: str = (
    "FR-16 load factor: 27 t per HGV trip; market share % stored as metadata; "
    "tkm derived from proxy sectorial data."
)
...
def calculate(
    ...
    *,
    ...
    """Compute Scope 3 Cat 9 EmissionRecords for downstream transport.
    ...
    Returns:
        List of ``EmissionRecord`` instances with ``sub_scope='Cat9'``.
    """
    records: list[EmissionRecord] = []
    for row in raw_rows:
        if int(row.get("categoria_s3", 0)) != 9:
            continue
        ...
        tco2e = (factor.value or Decimal("0")) * tkm * KG_TO_TONNE
        records.append(
            make_emission(
                ...
                disclosure_notes=(
                    f"Cat 9 distance-based: {row.get('sottocategoria', '')!s} "
                    f"({tkm} tkm via {factor.factor_id}). {_LOAD_FACTOR_NOTE}"
                ),
            )
        )
    return records
...
def _resolve_mode(sottocategoria: str) -> str | None:
    """Resolve a downstream Sottocategoria to a freight factor_id.

    Args:
        sottocategoria: Free-text Cat 9 sub-category label.
