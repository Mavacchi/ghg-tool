...
Reads from the ``calc.mv_intensity_metrics`` materialised view.  When the
view is absent (wave-3 pipeline not yet run), a stub empty response is
returned so the endpoint is testable without a live database.
...
@router.get(
    ...
    description=(
        "Returns intensity metrics (tCO2e per denominator unit) for the requested "
        "tenant, denominator type, and year range.  Reads from "
        "``calc.mv_intensity_metrics`` materialised view.  All authenticated roles "
        "may read.  A stub empty response is returned when the view is not yet "
...
async def get_intensity(
    tenant_id: Annotated[
        str,
        Query(description="Tenant UUID (required)", min_length=1),
    ],
    ...
    """Return intensity metrics for the requested filters.

    Reads from ``calc.mv_intensity_metrics``.  Filters are pushed down to
    the DB query (REV-021/022 query-pushdown principle) — no in-Python
    post-filtering.

    Args:
        tenant_id: Tenant UUID string for RLS isolation.
    ...
    try:
        ...
        return IntensityResponse(
            ...
            correlation_id=correlation_id,
            as_of=dt.datetime.now(tz=dt.UTC),
            **{"_note": "calc.mv_intensity_metrics not yet available — created in wave 3"},
        )
